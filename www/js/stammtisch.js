...
function parseCat(cat) {
    if (cat.match(/^(1|2|3|[qQ]\d(?:[-+])?|BNP:\d+|\?)(?:(::(?:night|play|mask|inwork|xmas|bomb|trailer=no|temp))+;?)?/)) {
        ...
        var attribs = {};
        attribString.replace(/^::/, "").split("::").forEach(function(x) {
            attribs[x] = true;
        });
        return {
            cat: cat,
...
/*
    ...
    L.Map.include({
        ...
        my_locate: function( /*Object*/ options) {
            ...
            if (options.watch) {
                this._my_locationWatchId = navigator.geolocation.watchPosition(onResponse, onError, options);
            } else {
...
function addLocators() {
        ...
        onAdd: function(map) {
            ...
            container.onclick = function() {
                if (confirm("Clear track?")) {
                    ...
                    trackPolyline = L.multiPolyline(trackSegs.polyline, {
                        color: '#f00',
                        weight: 2,
                        opacity: 0.7
                    }).addTo(map);
    ...
    map.on('locationerror', function(e) {
        locationFoundOrNot('locationerror', e);
    });
