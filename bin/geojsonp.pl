...
sub geojsonp {
    ...
    foreach my $s (@$stammtisch) {
	   ...
	   $hash->{ $name_norm } = homepage($location->{$name_norm}->{'name'}, $location->{$name_norm}->{'homepage'}) .
...
sub geojson_obj {
 ...
 my @list;
  ...
  foreach my $key (sort keys %$description) {
     my $h = { "geometry" => { "coordinates" => [ "13.35835", "52.52327" ], "type" => "Point" },
               "properties" => { "cat" => "X",  "name" => "<b>Restaurant Neumann's</b> : Alt-Moabit 126 : Moabit<br>\n* 2024-01-31 : 7" },
               "type" => "Feature"
	     };
     push @list, $h;
  }
